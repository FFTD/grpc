 ...
 public:
  ...
  std::shared_ptr<ChannelCredentials> GetChannelCredentials(
    ...
    if (type == grpc::testing::kInsecureCredentialsType) {
      ...
      string test_root_cert_str(test_root_cert);
      SslCredentialsOptions ssl_opts = {test_root_cert_str, "", ""};
      args->SetSslTargetNameOverride("foo.test.google.fr");
      std::shared_ptr<ChannelCredentials> credential_ptr = grpc::SslCredentials(ssl_opts);
  ...
  std::shared_ptr<ServerCredentials> GetServerCredentials(
    ...
    if (type == grpc::testing::kInsecureCredentialsType) {
      ...
      if (!custom_server_key_.empty() && !custom_server_cert_.empty()) {
        ...
        string test_server1_key_str(server_key);
        string test_server1_cert_str(server_cert);

        SslServerCredentialsOptions::PemKeyCertPair pkcp = {
            test_server1_key_str, test_server1_cert_str};
        ssl_opts.pem_key_cert_pairs.push_back(pkcp);
        std::shared_ptr<ServerCredentials> credential_ptr = SslServerCredentials(ssl_opts);
